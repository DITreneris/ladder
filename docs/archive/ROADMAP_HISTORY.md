# ROADMAP history archive — Corporate Ladder

**Purpose:** Historical gates, release-train snapshots, and research passes migrated from [ROADMAP.md](../ROADMAP.md) (2026-06-11). Forward-looking status lives in ROADMAP only.

**Doc map:** [DOCS_INDEX.md](../DOCS_INDEX.md) · **Ship detail:** [CHANGELOG.md](../CHANGELOG.md)
...
| **v2.1.0** | Retention sprint: Director rank @ 20y, beat-your-gap, challenge link, adaptive rookie ramp | **In progress** (2026-06-11) | F&F CONDITIONAL GO data — [FF_REVIEW](docs/FF_REVIEW_2026-06-14.md) §D |
...
| Spawn weight overrides (v1.7) | 5 presets (incl. Synergy Sprint v1.9); engine reads `dailyModifier` |
...
| OG / link preview (v1.8.2+) | `public/og.png`, meta tags; v1.9+ **`index, follow`** + crawler assets (`sitemap.xml`, `llms.txt`) |
...
| Synergy Sprint (v1.9) | 60s wall-clock cap; score = years at buzzer | Synergy Sprint |

---

## Shipped baseline (v1.9 → v2.0 + Unreleased)

Inventory for releases after v1.8.5 — do not regress without spec update. Detail: [CHANGELOG 1.9.0](CHANGELOG.md#190---2026-06-14), [2.0.0](CHANGELOG.md#200---2026-06-14), `[Unreleased]`.

### Mechanics

| Item | Notes |
|------|--------|
| Synergy Sprint preset (v1.9) | 5th daily preset; 60s wall-clock cap; `sprint_mode` on submit |
| Near-miss wince (v1.9) | Safe-side tap past imminent hazard; reduced-motion safe |
| Corporate triage rung (v2.0) | Manager+ every ~16 rungs; P1 backlog lane bias × 3 rungs — [V2_TRIAGE_SPIKE.md](docs/V2_TRIAGE_SPIKE.md) |
| Background tab fairness (v2.0) | Energy drain pauses while document hidden |
| Score plausibility cap (v2.0) | API rejects outlier years/rungs/session duration |
| Sprint mode trust (v2.0) | API validates `sprint_mode` vs UTC daily preset |

### Graphics

| Item | Notes |
|------|--------|
| Avatar emoji picker (v1.9) | Home Employee Badge; localStorage |
| Sprint HUD chip (v1.9) | Wall-clock countdown on Synergy Sprint days |
| Home gameplay preview (v1.9) | `#homeGameplayPreview` dodge/climb card |
| PA logo footer (Unreleased) | Official Prompt Anatomy logo in `#homeBrandFooter` |
| Sound FAB light theme (Unreleased) | Visible in Telegram light theme |

### Animation

| Item | Notes |
|------|--------|
| Near-miss wince (v1.9) | `.near-miss-wince` on player emoji |
| In-run BGM (v1.9) | Chorus loop from Manager promo; **Home silent** (lounge BGM removed) |
| News ticker scroll (v1.9) | Restored; static for reduced-motion / capture modes |

### Satirical view

| Item | Notes |
|------|--------|
| Tier A copy trim (Unreleased) | Shorter home/how-to; game-over flavor without outer quotes |
| Revive CTA (Unreleased) | “Mandatory HR Training” — AdsGram rewarded, one per run |
| SEO/crawler copy (Unreleased) | Dodge meetings / org chart; `@CorporateLadder_bot` canonical |

### Platform

| Item | Notes |
|------|--------|
| Session token auth (v2.0) | `/auth/me` → `session_token`; `POST /leaderboard/me` highlight |
| Leaderboard fetch window (Unreleased) | Up to 2000 runs before best-per-user aggregation |
| AdsGram rewarded revive (Unreleased) | Optional; `VITE_ADSGRAM_*` env vars |
| TON Builders analytics (Unreleased) | `@telegram-apps/analytics`; no-op without token |
| Live SEO smoke (Unreleased) | `verify:seo:live` in CI |

---

<details>
<summary>Historical gate — v1.8.2 (F&F-ready, superseded)</summary>
...
## v1.9.0 — Shipped (2026-06-14)

**Tagged and live.** Detail: [CHANGELOG 1.9.0](CHANGELOG.md#190---2026-06-14).

| Item | Status |
|------|--------|
| Near-miss wince | Shipped |
| Synergy Sprint preset | Shipped |
| F&F UX pack (40-rung hints, LB self-row, avatar) | Shipped |
| OG adopt + SEO shell | Shipped |
| Soft drain cap @ ~20y | Deferred |
| Clean-climb streak | Deferred |

---

<details>
<summary>v1.9.0 planning notes (pre-ship archive)</summary>

## v1.9.0 — Near-miss wince + Synergy Sprint (planning)
...
| Beat-your-gap on home | Satire + retention | **Adopted → v2.1.0** (retention trigger met 2026-06-11) |
| Shift-specific retry tips | Satire | Extend `RETRY_TIPS` per daily preset |
| Share challenge link | Platform + satire | **Adopted → v2.1.0** (`t.me/bot?startapp=c_*`) |
| Server-seeded daily + modifier LB | Mechanics + platform | Daily DAU threshold |
| Adaptive difficulty from career high | Mechanics + retention | **Adopted → v2.1.0** (F&F: beginners churn before 10y — external median ~5y) |
| Expanded rank tree (Junior, VP, C-suite steps) | Satire + mechanics | Single Director rank @ 20y **adopted → v2.1.0** (scope exception); further steps stay out |
